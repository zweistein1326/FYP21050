    ...
    try {
        ...
        if(credential){
            if(credential.owner == fromAddress){
                ...
                return res.status(200).json({message:`${fromUser.username} does not have permission to transfer object`, success:false})
            }
        }
        else{
...
router.post('/addViewer', async(req,res,next)=>{
    ...
    try{
        ...
        if(credential.owner == senderId){
            credential.addViewer(viewerId);
            const newCredential = AwesomeCredentials.getCredentialById(credentialId);
            return res.json({credential, success:true})
        }
        return res.json({message:'User does not have correct permissions', success:false});
    }
    catch(e){
        return res.status(200).json({message:e.message, success:false})
    }
